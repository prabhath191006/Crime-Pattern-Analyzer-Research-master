...
for file in os.listdir(folder):
    if file.startswith("district_"):
        ...
        graph = set(train_counts.head(top_n).index)
        rule = set(train_counts[train_counts > train_counts.mean()].index)

        pred = set(list(graph)[:top_n])  
        actual = set(test_counts.head(top_n).index)

        acc = (len(pred & actual) / len(actual)) * 100 if len(actual) else 0
        results.append({"Dataset": file, "Human_AI": round(acc, 2)})

pd.DataFrame(results).to_csv("human_ai_results.csv", index=False)